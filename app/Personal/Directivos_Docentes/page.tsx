...
import { Alice , Source_Serif_4} from 'next/font/google';
import ImagenesDocentes from './ImagenesDocentes';
import ImagenesSlider from './ImagenesSlider';
...
const page = () => {
  ...
  const [SliderIMGSOG, setSliderOG] = useState<string[]>([""])
  const [SliderIMGS, setSlider] = useState<string[]>([""])
  const [IndexSlide, setSlide] = useState<number>(STARTINDEX)
  const PuedeNext = IndexSlide < SliderIMGSOG.length - 1
  const PuedePrev = IndexSlide > 0 
  ...
  useEffect(() => {
    ...
    async function obtenerImagenesSlider() {
      try{
        const response = await fetch('/api/Slider/DirectivosDocentes');
        const data = await response.json();
        const arreglobase = data.filenames;
        const arreglofinal = [arreglobase[arreglobase.length - 2], arreglobase[arreglobase.length - 1], ...arreglobase, arreglobase[0], arreglobase[1]]
        setSlider(arreglofinal);
        setSliderOG(arreglobase);
        console.log("Slider", SliderIMGS)
  ...
  useEffect(() => {
    console.log("cambio indexx", IndexSlide)
  }, [IndexSlide])
  
   useEffect(() => {
    console.log("cambio indexx", offsetX)
  }, [offsetX])

  useEffect(() => {
    if (itemsRef.current && itemsRef.current.at(STARTINDEX)) {
      const currentItem = itemsRef.current.at(STARTINDEX);
      if (currentItem) {
        currentItem.classList.add("opene");
      }
    }
  }, [SliderIMGS])
  ...
  const NextCard = () => {
    ...
    if (!PuedeNext && container){
      ...
      itemsRef.current.at(IndexSlide)?.classList.remove("opene");
        itemsRef.current.at(IndexSlide)?.classList.add("cerradoo");
         itemsRef.current.at(0)?.classList.remove("cerradoo");
        itemsRef.current.at(0)?.classList.add("opene");
      ...
      setTimeout(() => {
        ...
        offsetX.set(offsetX.get() + nextWidth * 7 )
      }, 300)
      ...
      const nextWidth = itemsRef.current
        ...
        itemsRef.current.at(IndexSlide)?.classList.remove("opene");
        itemsRef.current.at(IndexSlide)?.classList.add("cerradoo");
         itemsRef.current.at(IndexSlide + 1)?.classList.remove("cerradoo");
        itemsRef.current.at(IndexSlide + 1)?.classList.add("opene");
  
        offsetX.set(offsetX.get() - nextWidth)
        console.log("Inicio", IndexSlide)
  ...
  const PrevCard = () => {
    ...
    if (!PuedePrev && container){
      ...
      if (nextWidth === undefined) return
        ...
        itemsRef.current.at(IndexSlide)?.classList.remove("opene");
        itemsRef.current.at(IndexSlide)?.classList.add("cerradoo");
        itemsRef.current.at(6)?.classList.remove("cerradoo");
        itemsRef.current.at(6)?.classList.add("opene");
      offsetX.set(offsetX.get() + nextWidth )
      setSlide(6)
  ...
  return (
   <main> 
    <header className='Hhheader'>
      ...
      <section className='container-sliderD'>

              <span className='btn-der' onClick={() => NextCard()}>
                  <svg fill="#004AAD" height="200px" width="200px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 330.002 330.002" xmlSpace="preserve"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path id="XMLID_19_" d="M229.966,0.847c-6.011-2.109-12.698-0.19-16.678,4.784L93.288,155.635 c-4.382,5.478-4.382,13.263,0.001,18.741l120,149.996c2.902,3.628,7.245,5.63,11.716,5.63c1.658,0,3.336-0.276,4.962-0.847 c6.012-2.108,10.035-7.784,10.035-14.154v-300C240.001,8.63,235.978,2.955,229.966,0.847z"></path> </g></svg>
              </span>
              
              <span className='btn-izq' onClick={() => PrevCard()}>
                  <svg fill="#004AAD" height="200px" width="200px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 330.002 330.002" xmlSpace="preserve"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path id="XMLID_19_" d="M229.966,0.847c-6.011-2.109-12.698-0.19-16.678,4.784L93.288,155.635 c-4.382,5.478-4.382,13.263,0.001,18.741l120,149.996c2.902,3.628,7.245,5.63,11.716,5.63c1.658,0,3.336-0.276,4.962-0.847 c6.012-2.108,10.035-7.784,10.035-14.154v-300C240.001,8.63,235.978,2.955,229.966,0.847z"></path> </g></svg>
              </span>
          ...
          className='Container-cardds' ref={containerRef}>{
              
              !loading?
              <ImagenesSlider SliderIMGS={SliderIMGS} Iref={itemsRef} />
