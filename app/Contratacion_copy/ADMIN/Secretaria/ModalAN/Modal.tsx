...
const Modal = ({id, Name, cerrado, HandleClick, NameContract, NameLic}: { id: number; Name: string ;cerrado: boolean; HandleClick: () => void ; NameContract: string; NameLic: string}) => {
...
const [nombreContrato, setnombreCont] = useState<string>("");
const [FechaInner, setFechaInner] = useState<string>("")


  useEffect(() => {
    setnombre(NameLic)
    setnombreCont(NameContract)
    setFechaInner("")

    console.log("contraaato", NameContract)
    console.log("Licitacion", NameLic)
  },[cerrado])
  ...
  function EliminarElemento(FileDeleted: File |  null){
    ...
    return (
      <>
          <div id={id.toString()} className={`overlay ${cerrado ? 'hidden anim_fade-out' : 'showed'}`}>
              <div className={`contenedor-modalN ${cerrado ? 'hidden-content anim_fade-outmove' : ''}`}>
                            ...
                            async (e) => {
                             ...
                             try{
                              const form = new FormData()
                                ...
                                form.set('Contract', nombreContrato)
                              ...
                              <div className='contenedorInfo' >
                                {
                                  UplFiles.map((Archivo) => {
                                    return(
                                      <C_Archivo id={id} enviado={Enviado} key={Archivo.lastModified} File={Archivo} Ffunction={EliminarElemento}/>
                                    );
                                  })
                                }
                          ...
                          <div className='contenedor-botones'>
                            <button type='button' disabled={Enviado} className={`${Enviado ? 'noclickeable' : 'botoneleccion'}`}>
                              ...
                              <input type='file' className={`${Enviado ? 'noclickeable' : ''}`} disabled={Enviado} onChange={(e) => {
                                console.log("cambio :p", NameContract)
